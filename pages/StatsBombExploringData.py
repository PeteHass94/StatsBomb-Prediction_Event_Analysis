...
st.title('⚽ StatsBomb Data Explorer')
st.caption('Template created by Ana Beatriz Macedo in Data Basics Masterclass')
...
def get_competitions():
    """Get competitions data from StatsBomb API.
    Only includes specific competition IDs and season IDs.
...
def main():
    ...
    st.expandable_text = st.expander("See all competitions")
    ...
    st.expandable_text = st.expander(f"See all matches for {selected_competition_name} {selected_year_name}")
    with st.expandable_text:
    ...
    st.subheader(f"Match Events for {selected_match_name} ({match_id})")
    st.expandable_text = st.expander("See all match events")
    with st.expandable_text:
